...
> - **Models:** spike result — **MLX is not viable for the encoder / token-classification family** (`mlx-lm` ships no encoder architectures and only a causal-LM loss). The `kp-deid` encoder finetunes therefore run **`transformers` + `peft` on CPU (smoke) / CUDA on a DO droplet (real run)**; `mdeberta-v3-base` LoRA is verified end-to-end. MLX-first still holds for *inference* and the causal-LM/anonymizer track. Dependency extras de-conflicted (`.[hf]`/`.[mlx]` each resolve to transformers ≥ 5.0).
...
| **Models** | CPU/CUDA path (mDeBERTa-280m + LoRA — MLX not viable for encoders); KpModelAdapter; SDK `extract_pii`; first kp rows on RO real-skeleton | XLM-R-560m (GPU-burst gated); coverage-aware "protector" objective; T1 langs; kp-anon + SDK full | T2/T3 under-served langs; decision-gated from-scratch; clinical on credentialed gold |
